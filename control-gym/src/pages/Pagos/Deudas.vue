<template>
  <div>
    <div class="q-pa-xl">
      <div v-if="!datos.length" class="text-center text-white text-weight-bold text-h3">
        No hay deudas disponibles
      </div>
      <q-table
        v-if="datos.length"
        title="Usuarios Deudas"
        class="table q-mt-md"
        dark
        color="white"
        :data="datos"
      ...
      >
        ...
        <template v-slot:body="props">
          <q-tr @click="selected(props.row.id)" :props="props" class="cursor-pointer">
      ...
      <q-dialog v-model="open">
        <q-card class="bg-drawer" style="width: 700px; max-width: 80vw;">
          ...
          <q-card-section class="q-pt-none">
            ...
            >
              <template v-slot:top-right>
                <q-input dark borderless dense debounce="300" v-model="filter2" placeholder="Buscar">
                  <q-icon slot="append" name="search" />
                </q-input>
              </template>
                ...
                <q-tr :props="props" class="cursor-pointer">
                  ...
                  <q-td key="actions" :props="props">
                    ...
                    <q-btn
                      ...
                      @click="ver(props.row)"
                    >
                      <q-tooltip content-class="color-orange">Ver Más</q-tooltip>
                    </q-btn>
                  </q-td>
                </q-tr>
              </template>
      ...
      <q-dialog v-model="view" persistent>
        <q-card class="bg-drawer">
          <q-btn flat round icon="reply" color="white" @click="form = {}, view = false" v-close-popup class="q-ml-xs q-mt-xs"/>
...
export default {
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'cedula', align: 'right', label: 'Cedula', field: 'cedula', sortable: true },
        { name: 'numero_casa', align: 'right', label: 'Correo', field: 'numero_casa', sortable: true }
      ],
      columns_adelantos: [
        {
          name: 'amount',
        ...
        { name: 'createdAt', align: 'center', label: 'Fecha de creación', field: 'createdAt',sortable: true},
  ...
  methods:{
    async getList() {
      this.$q.loading.show()
      await this.$axios2.post('deudas/get_users', {
    ...
    async selected(id) {
      this.$q.loading.show()
      await this.$axios2.post('deudas/selected', {
        id: id,
        condominium: this.condominium 
      }).then(res => {
    ...
    async eliminar(id) {
      this.$q.dialog({
        ...
        await this.$axios2.delete('adelantos/' + id).then(async val => {
          this.$q.notify({
